//Realizar un programa que sea una tienda donde podamos almacenar productos y precios y mostrar una factura al cliente

#include "iostream"
using namespace std;
const int longCade=20;

struct
{
    char nombreArticulo [longCade + 1][50];
    int cantidad[50];
    float precio[50];
...
void informacion(int nArticulos)
{
    ...
    for (int i = 0; i < nArticulos; i++)
    {
        ...
        cout<<"Ingrese el precio por unidad de su articulo: "<<endl;
...
{
    cout<<"\t Resultados de los productos que se esta pidiendo: "<<endl;
    for (int i = 0; i < nArticulos; i++)
    {
        cout<<"Producto: "<<tienda.nombreArticulo[i]<<endl;
        cout<<"Cantidad: "<<tienda.cantidad[i]<<endl;
        cout<<"Costo por el articulo: $"<<tienda.precio[i]<<endl;;
    }
    return;
}
void total(int nArticulos)
{
    ...
    cout<<"\tTotal sin iva....es $"<<total<<endl;
    iva = total * 0.13;
    cout<<"\nIva....$"<<iva<<endl;
